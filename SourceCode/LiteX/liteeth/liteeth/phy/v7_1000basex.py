...
class LiteEthSPDMRX(LiteXModule):
  def __init__(self):
  	...
  	self._register_SPDM_2 = CSRStorage(32, name = "SPDM_register_2", description = "SPDM_Control")
  	...
  	self._register_SPDM_5 = CSRStorage(8, name = "SPDM_FIFO", description = "SPDM_Control")
...
class V7_1000BASEX(LiteXModule):
    ...
    def __init__(self, refclk_or_clk_pads, data_pads, sys_clk_freq, refclk_freq=200e6, with_csr=True, rx_polarity=0, tx_polarity=0):
        ...
        self.reset = Signal()
        self.submodules._register_spdm = LiteEthSPDMRX()
        if with_csr:
            self.add_csr()
