...
uint8_t m_send_receive_buffer[4*LIBSPDM_MAX_MESSAGE_BUFFER_SIZE]; 
...
uint32_t m_support_hash_algo = //SPDM_ALGORITHMS_BASE_HASH_ALGO_TPM_ALG_SHA3_512 |
...
{ 
    ...
    *message_size = (size_t)ethphy__register_spdm_SPDM_responder_control_read();
...
void *spdm_requester(void)
{
    ...
    /* Send GET_MEASUREMENT to query the total number of measurements available. */
    status = libspdm_get_measurement (spdm_context, NULL, request_attribute, SPDM_GET_MEASUREMENTS_REQUEST_MEASUREMENT_OPERATION_ALL_MEASUREMENTS,
       slot_id, content_changed,
       &number_of_blocks,
       &measurement_record_length,
       measurement_record
    ); 
    
    printf("libspdm_get_measurement - 0x%x\n", (uint32_t)status);
    
    status = libspdm_start_session (
       spdm_context,
       false, // KeyExchange without PSK.
       SPDM_CHALLENGE_REQUEST_ALL_MEASUREMENTS_HASH,
       slot_id, 0,
       &session_id,
       &heartbeat_period,
    ...
    printf("libspdm_start_session - 0x%x\n", (uint32_t)status);      
