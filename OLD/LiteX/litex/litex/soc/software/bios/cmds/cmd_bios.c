...
static void leds_handler(int nb_params, char **params)
{
	...
	//ethphy_rx__register_spdm_SPDM_register_write(value); */
}
...
static void spdm_responder_handler(char **params)
{
	spdm_responder();
}
define_command(spdm_responder, spdm_responder_handler, "Set BIOS to use SPDM as responder", SYSTEM_CMDS);
#endif  

#ifdef CSR_ETHPHY_BASE
static void spdm_requester_handler(char **params)
{
	spdm_requester();
}
define_command(spdm_requester, spdm_requester_handler, "Set BIOS to use SPDM as requester", SYSTEM_CMDS);
#endif  
